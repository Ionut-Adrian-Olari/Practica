...
const Game = () => {
  ...
  const winner = calculateWinner(history[stepNumber]);
  const XsauO = xIsNext ? "X" : "O";
  const { squares, onClick } = React.useContext(GameContext);

  const goToGameStart = () => {
    setHistory([initialContext.squares]);
    setStepNumber(0);
    setXIsNext(true);
  };
  const handleClick = (i) => {
  ...
  return (
    ...
    >
      ...
      <div className="info-wrapper">
        ...
        <h3>{status}</h3>
        {stepNumber === 9 && (
          <Popup goToGameStart={goToGameStart} winner={winner} XO={winner} />
        )}
        {winner && (
          <Popup goToGameStart={goToGameStart} winner={winner} XO={winner} />
        )}
